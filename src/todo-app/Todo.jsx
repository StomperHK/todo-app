...
export function Todo() {
  ...
  function addTodoDispatcher(todoId, todoText, todoDescription, todoDate, todoPriority) {
    dispatch({
      type: "add",
      todoId: todoId,
      todoText,
  ...
  function editTodoDispatcher(todoId, newTodoText, newTodoDescription, newTodoDate, newTodoPriority) {
    dispatch({
      type: "edit",
      todoId,
      newTodoText,
    ...
    <>
      ...
      <div className=" w-fit m-auto mt-10 mb-10 relative">    {/* position wrapper */}
        ...
        <main className="order-1 max-w-xl w-[90svw] p-4 border-2 border-zinc-500 rounded-md bg-zinc-800 shadow-normal">
          ...
          <ul className={`${todoList} rounded overflow-hidden`}>
            {
